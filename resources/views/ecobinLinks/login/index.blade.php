<x-sidebar-layout>
    ...
    <div class="">
        ...
        <table id="search-table" class="table-auto w-full border-collapse">
            ...
            <tbody>
                <!-- Example static row -->
                @foreach ($ecobinLoginLinks as $ecobinLoginLink)
                   <tr>
                        <td class="font-medium text-gray-900 whitespace-nowrap dark:text-white">{{ $ecobinLoginLink->id }}</td>
                        <td style="overflow-wrap:anywhere;">{{$ecobinLoginLink->loginLink}}</td>
                        <td>
                            <div class="flex">
                                <a href="{{ route('ecobinLinks.login.edit', $ecobinLoginLink->id) }}" class=" py-2 px-8 rounded-md bg-green-700 text-white mr-5">Edit</a>
                                <!-- Delete Form with Confirmation -->
                                {{-- <form action="{{ route('ecoBinLoginLink.delete', $ecobinLoginLink->id) }}" method="POST" class="delete_form">
                                    @csrf
                                    @method('delete')
