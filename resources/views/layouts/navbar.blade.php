    ...
    <div class="flex justify-between pt-3 w-[92%] mx-auto items-center">
        ...
        <div class="nav-links md:static absolute md:bg-transparent bg-[#f9fff5] rounded-md md:min-h-fit min-h-[30vh] justify-center left-0 top-[-100%] md:w-auto w-full flex items-center ml-auto px-5 transition-all duration-500 ease-in-out ">
            ...
            <li class="text-[#016262] px-1 py-1 text-md md:w-25 md:h-8 lg:w-25 lg:h-9 md:cursor-pointer cursor-pointer rounded-sm text-md transition delay-30 duration-300 ease-in-out font-bold">
                <a class="<?= $_SERVER['SCRIPT_NAME'] == "/view/productAndService.php" ? 'bg-[#e6e6e5] text-[#016262] py-2 px-2 rounded-sm' : 'hover:bg-[#e6e6e5] py-2 px-2 rounded-sm' ?>" href="{{ route('about-us') }}">About us</a>
            </li>

            <!-- SOLUTIONS NAV -->
            <li id="dropdownHoverButton" data-dropdown-toggle="dropdownHover" data-dropdown-trigger="hover" class="btn text-[#016262] px-1  py-1 text-md md:w-25 md:h-8 lg:w-25 lg:h-9  md:cursor-pointer cursor-pointer rounded-sm text-md transition delay-30 duration-300 ease-in-out font-bold flex  justify-center items-center" type="button"><a class="<?= $_SERVER['SCRIPT_NAME'] == "/view/aboutUs.php" ? 'bg-[#e6e6e5] text-[#016262] py-2 px-2 rounded-sm' : 'hover:bg-[#e6e6e5] py-2 px-2 rounded-sm' ?>"  href="{{ route('about-us')}}">Solutions</a><svg class="w-2.5 h-2.5 ms-3 flex" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 10 6">
